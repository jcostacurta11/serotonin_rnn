...
# CODE FOR FOUR-NEURON MODEL
# potential nonlinearities: identity (defined below), jax.nn.relu, jax.nn.sigmoid

def identity(x):
    return x

def sero_prob_nmrnn(params, x0, z0, s, inputs, tau_x, tau_z, nln):
    """
    Arguments:
    - params
    - x0
    ...
    - tau_z   : decay constant
    - nln   : nonlinearity
    """

    J = params["recurrence_matrix"]       # N x N
    W = jnp.diag(params["nm_recurrence_diag"])      # dim_nm 
    C = params["readout_weights"]   # O x N
    rb = params["readout_bias"]             # O
    B_xz = params["B_xz"]         # N x dim_nm
    B_xu = params["B_xu"]      # N x U
    B_zx = params["B_zx"]         # dim_nm x N
    B_zu = params["B_zu"]      # dim_nm x U
    d = params["d"]      # 2
    N = J.shape[0]
    M = W.shape[0]
    ...
    # make sure components of z can only excite one subpopulation of x
    B_xz = B_xz.at[ : N//2, 1].set(0)
    B_xz = B_xz.at[N//2 : , 0].set(0)
    B_xz = jnp.abs(B_xz)

    B_zu = B_zu.at[ : M//2, 1].set(0)
    B_zu = B_zu.at[M//2 : , 0].set(0)
    B_zu = jnp.abs(B_zu)

    # construct B_zx
    d = jnp.abs(d) + 1 # d > 1
    d_opts = jnp.array((d, 1/d))
    ind = s.astype(int)

    B_zx = inh(B_zx.at[1,1].set(d_opts[ind])) # set MPOA -> 5-HT_MPOA to be scaled down for mothers
    ...
    def _step(x_and_z, u):
        ...
        zp = z
        z = (1.0 - (1. / tau_z)) * zp
        z += (1. / tau_z) * nln(W @ zp + B_zu @ u + B_zx @ x ) # recurrent dynamics for z

        # update x
        xp = x # hold onto previous value
        x = (1.0 - (1. / tau_x)) * xp # decay term
        x +=  (1. / tau_x) * nln(J @ x + B_xz @ z) # update (without contextual input)
...
batched_sero_prob_nmrnn = vmap(sero_prob_nmrnn, in_axes=(None, None, None, 0, 0, None, None, None))

def batched_sero_prob_nmrnn_loss(params, x0, z0, s, batch_inputs, tau_x, tau_z, batch_targets, batch_mask, nln):
    ys, _, _ = batched_sero_prob_nmrnn(params, x0, z0, s, batch_inputs, tau_x, tau_z, nln)
    return jnp.sum(((ys - batch_targets)**2)*batch_mask)/jnp.sum(batch_mask)

def random_sero_prob_nmrnn_params(key, u, z, n, o):
    """Generate random parameters

    Arguments:
    u:  number of inputs
    m:  number of neurons in nm subnetwork
    ...
    return {'recurrence_matrix' : jr.normal(skeys[0],(n,n))*0.1,
            ...
            'B_xz' : jr.normal(skeys[3], (n,z))*ifactor,
            'B_xu' : jr.normal(skeys[4], (n,u))*ifactor,
            'nm_recurrence_diag' : jr.normal(skeys[5], (z,))*0.1,
            'B_zx' : jr.normal(skeys[6], (z,n))*0.1,
            'B_zu' : jr.normal(skeys[7], (z,u))*0.1,
            'B_zz' : jr.normal(skeys[8], (z,z))*0.1,
            'd' : jr.normal(skeys[9])}

def fit_sero_prob_nmrnn(inputs, targets, loss_masks, params, optimizer, x0, z0, s, num_iters, tau_x, tau_z, nln=jax.nn.relu,
    ...
    def _step(params_and_opt, input):
        ...
        loss_value, grads = jax.value_and_grad(batched_sero_prob_nmrnn_loss)(params, x0, z0, s, inputs, tau_x, tau_z, targets, loss_masks, nln)
...
# data generation for phasic serotonin
def data_generation(key, num_samples):
    ...
    u2_1 = jr.normal(keys[2], (num_samples, 100)) * 0.05 + placed_ramp
    u2_2 = jr.normal(keys[3], (num_samples, 100)) * 0.05 + placed_ramp
    ...
    s = jnp.zeros((num_samples,)) 
    s = s.at[~mpoa_select].set(jnp.array([0.])) # 0 = virgin trial
    s = s.at[mpoa_select].set(jnp.array([1.])) # 1 = mother trial

    u2 = jnp.where(mpoa_select[:,None,None], 
                  jnp.stack((u2_1, u2_2), axis=-1),
                  jnp.stack((u2_1, u2_2), axis=-1))
...
n_nm = config['M']
keyn = config['keyind']
n_train_iters = config['num_full_train_iters']

init_params = random_sero_prob_nmrnn_params(jr.PRNGKey(keyn), 2, n_nm, n_neurons, 3)
u,s,p = data_generation(jr.PRNGKey(13), 100)
...
z0 = jnp.ones((n_nm,))*0.1

params, losses = fit_sero_prob_nmrnn(u, p, jnp.ones((100,100,1)), init_params, optimizer, x0, z0, s, n_train_iters, tau_x, tau_z, nln=nln_func, wandb_log=False)

# Code below is for saving model to Weights and Biases and generating diagnostic figures

# log_wandb_model(params, "sero_nmrnn_n{}".format(config['N']), 'model')
# wandb.log({"d":jnp.abs(params['d'])}, commit=True)
...
# for ind in [2,5]:
#     dict = {2: "virgin", 5: "mother"}

#     y, x, z = sero_prob_nmrnn(params, x0, z0, s[ind], u[ind], tau_x, tau_z, nln=nln_func)
    
#     # targets/output plot
#     fig, ax = plt.subplots()
#     ax.plot(y, label="RNN output")
#     ax.plot(p[ind], label="Target")
#     ax.plot(u[ind], label="inputs")
...
#     # nm plot
#     fig, ax = plt.subplots()
#     ax.plot(z[:,:N//2], c="blue", label="z_BNST")
#     ax.plot(z[:,N//2:], c="red", label="z_mPOA")
#     ax.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
#     plt.tight_layout()

#     wandb.log({f'model_nm_activity_{dict[ind]}': wandb.Image(fig)}, commit=True)